...
exports.handler = function(event, context, callback) {
  ...
  if (queryStringParameters.challenge) {
    const msg = "Success: verification request received and responded to appropriately.";
    callback(null, {
      statusCode: 200,
    ...
    if (process.env.NETLIFY_BUILD_HOOK_URL) {
      const msg = "Success: webhook received from Dropbox and forwarded to netlify!";
      ...
      const msg = "Failed: the `NETLIFY_BUILD_HOOK_URL` environment variable is missing.";
      callback(null, {
        statusCode: 200,
        body: msg,
      });
