...
module.exports = (api, options, rootOptions) => {
  ...
  try {
    ...
    const apiLowdbPath = path.join(apiRootDir, "lowdb/index.ts");
    console.log("apiLowdbPath is:" + apiLowdbPath);
    if (!fs.existsSync(apiLowdbPath)) {
      files[apiLowdbPath] = `${templatesRoot}/api/lowdb/index.ts`;
    }
    const apiGlobalPath = path.join(apiRootDir, "globals.ts");
    console.log("apiGlobalPath is:" + apiGlobalPath);
    if (!fs.existsSync(apiGlobalPath)) {
      files[apiGlobalPath] = `${templatesRoot}/api/globals.ts`;
    }

    // util/helper
    const utilPath = path.join("./src", "util/index.ts");
    if (!fs.existsSync(utilPath)) {
      files[utilPath] = `${templatesRoot}/util/index.ts`;
    }
    ...
    if (!fs.existsSync(moduleBaseDir)) {
      ["index", "actions.nedb", "actions.lowdb", "mutations", "getters"].forEach(template => {
    ...
    // pluginBaseDir @/store/plugins/*
    const pluginBaseDir = path.join(storeRootDir, "plugins");
    ...
    if (!fs.existsSync(pluginBaseDir)) {
      ["index", "persistentPlugin", "pathify"].forEach(template => {
        ...
        files[filePath] = `${templatesRoot}/store/plugins/${fileName}`;
      });
    } else {
    ...
    if (!fs.existsSync(routerBaseDir)) {
      ["index", "path"].forEach(template => {
        ...
        console.log("Router files generated in " + filePath);
        // to @/store/rotuer/*
