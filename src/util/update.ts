...
const vueStarterRepo: string = 'https://api.github.com/repos/linuxing3/awesome-manager'
...
async function update() {
  ...
  const diffResponse: AxiosResponse<any> = await axios.get(
    `${vueStarterRepo}/compare/${currentTag}...${latestTag}`
  )
  ...
  diffFiles.forEach((diffFile: IFile) => {
    ...
    const url: string = `https://raw.githubusercontent.com/devCrossNet/vue-starter/master/${
      diffFile.filename
    }`
